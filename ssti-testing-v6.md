...
- **`html` fonksiyonu — text/template vs html/template ayrımı için
  kullanılabilir bir sinyal (KRİTİK DÜZELTME — önceki `js` iddiası
  yanlıştı):** `html`, Go'nun resmi `ErrPredefinedEscaper` belgesinde
  **açıkça isimlendirilerek** `html/template`'te (belirli pipeline
  konumları hariç) kısıtlanan iki fonksiyondan biridir (diğeri
  `urlquery`). **`js` bu kısıtlama listesinde yer almaz** — resmi Go
  dokümantasyonu bu kısıtlamayı yalnızca `"html"` ve `"urlquery"` için
  tanımlar; `js`'in `html/template`'te aynı şekilde engellendiğine dair
  bir kanıt yoktur, bu yüzden **`js`'i bu fingerprint sinyaline dahil
  etme** — yalnızca `html` fonksiyonunu kullan:
  ```
  {{html "ssti"}}
  ```
  ...
  - **html/template'te:** `predefined escaper "html" disallowed in
    template` benzeri bir **hata** döner (bazı pipeline pozisyonlarında
    — örn. bir pipeline'ın **ortasında** veya unquoted attribute value
    context'inde; pipeline'ın **son** komutu olarak kullanımı Go 1.9+
    itibarıyla hâlâ izin verilebilir, bu yüzden önce birkaç pozisyon
    dene) — bu hata mesajının kendisi, hedefin `html/template`
