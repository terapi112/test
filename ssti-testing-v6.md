...
| Handlebars/Mustache | `{{6666*6666}}` çalışmaz (serbest expression evaluation yok — Mustache gerçekten logic-less, Handlebars ise helper-tabanlı logic sunar ama ham aritmetiği desteklemez) — §12.3 |
...
- Versiyon farkları önemlidir (örn. Twig 1.x'te bazı `_self`
  erişimleri farklı davranır; **ama sandbox modu Twig'de hiçbir
  sürümde varsayılan olarak açık değildir** — `SandboxExtension`
  bilinçli olarak eklenip etkinleştirilmesi gereken opsiyonel bir
  mekanizmadır, bkz. §12.1 Twig — Sandbox notu; Freemarker 2.3.30
  öncesi sürümlerde farklı sandbox bypass teknikleri geçerlidir —
  bkz. §12.2) — confirmation aşamasında versiyon-spesifik payload
  alternatifleri de denenmelidir.
...
- **Version differences:** Eski Twig sürümlerinde bazı `_self`
  erişimleri farklı davranır. **Not:** sandbox modu hiçbir Twig
  sürümünde varsayılan olarak açık değildir (regular/non-sandboxed
  Twig environment, template kaynağını her zaman trusted kabul eder);
  "sandbox artık varsayılan" varsayımı yanlıştır ve her hedefte
  ayrıca doğrulanmalıdır (yukarıdaki Sandbox notuyla tutarlı).
...
- **Delimiter:** `${ }` ifade (varsayılan/en yaygın syntax, ama
  **her zaman geçerli değildir** — `interpolation_syntax=square_bracket`
  ayarında `${...}` düz metin olarak kalır, bkz. hemen aşağıdaki
  configuration notu), `#{ }` **deprecated numerical interpolation**
  (yalnızca sayısal ifadeler için, uzun süredir deprecated), `[=...]`
  **square-bracket alternatif syntax** (configuration-dependent),
  `<# #>` directive, `<#-- -->` yorum.
...
- **Generic detection:**
  ...
  - `${6666*'6666'}` → sessizce boş dönmez; sayı×string tip
    uyuşmazlığından dolayı bir `TemplateException`/evaluation error
    beklenir (sürüm/`interpolation_syntax`'a göre hatanın gösterim
    şekli değişebilir). Bu yüzden "boş çıktı" değil, **hata sinyali**
    aranmalıdır — bu da zaten §7 Fingerprinting'teki error-signature
    tabanlı tespitle örtüşür.
...
- **Delimiter:** `${ }`, `th:*` attribute'ları, expression inlining
  için `[[...]]` / `[(...)]`.
- **Dialect ayrımı (KRİTİK — yanlış expression language seçimi
  false-negative üretir):** Thymeleaf'in kendisi (`StandardDialect`)
  `${...}` ifadelerini **OGNL** ile değerlendirir. Ama gerçek dünyada
  hemen her Spring Boot uygulaması `SpringTemplateEngine`'i kullanır,
  bu da otomatik olarak `SpringStandardDialect`'i devreye sokar — ve
  bu dialect `${...}` ifadelerini OGNL yerine **SpEL** ile
  değerlendirir. Yani bu bölümün başlığı "Spring Boot" olsa da,
  **hedef gerçekten Spring Boot ise varsayılan expression language
  SpEL'dir, OGNL değildir**; OGNL payload'ları yalnızca Thymeleaf'in
  Spring entegrasyonu olmadan (çıplak `StandardDialect`) kullanıldığı
  daha nadir hedeflerde geçerlidir. Confirmation denemelerinde önce
  SpEL sözdizimini (`T(java.lang.Runtime)...`), yalnızca o
  başarısız olursa ve hedefin Spring kullanmadığından şüpheleniliyorsa
  OGNL sözdizimini (`@java.lang.Runtime@getRuntime()`) dene.
- **Generic detection:** `${6666*6666}` → `44435556`;
  `${44444444-8888}` → `44435556` (aynı hedef değer, çıkarma probu).
...
- **Expression preprocessing (özel bir SSTI benzeri vektör):** Çift alt
  ...
  ```
- **RCE / confirmation:**
  - SpEL (Spring Boot / `SpringStandardDialect` — **önce bunu dene**):
    `${T(java.lang.Runtime).getRuntime().exec('id')}`
  - OGNL (yalnızca çıplak `StandardDialect`, Spring entegrasyonu
    yoksa): `${#rt = @java.lang.Runtime@getRuntime(),#rt.exec("id")}`
...
- **Delimiter:** `{{ }}`.
- **Mustache-türevi, helper-tabanlı tasarım (not: "logic-less" değil):**
  `{{6666*6666}}` **çalışmaz** — bu negative indicator,
  Handlebars/Mustache ailesine işaret eder. Handlebars'a "logic-less"
  demek yanıltıcıdır — bu terim gerçekte Mustache'e ait; Handlebars
  `#if`/`#unless`/`#each`/`#with` ve custom helper'lar gibi gerçek
  kontrol akışı mekanizmaları sunar (aşağıda kullanılan built-in
  helper discovery adımı zaten bunu doğrular). Ham aritmetik
  ifadelerin çalışmaması "logic yok" anlamına gelmez, yalnızca
  serbest expression evaluation'ın olmadığı, tüm logic'in helper'lar
  üzerinden yürüdüğü anlamına gelir.
