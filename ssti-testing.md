...
SSTI sink sinyali var mı?
  ...
  └─ Evet → §3 Source→Sink + §4 Sink Discovery
              ...
              └─ Evet → §5 Generic Detection → §6 Context
                          ...
                          ↓
                          Bağımsız evidence kategorisi var mı?
                          ├─ Evet → Confirmed → Impact → §14 Reporting
                          └─ Hayır → yeni evidence kategorisi dene; bütçe dolarsa dur
```
...
- **Medium/weak signal** → tek başına confirmed kabul etme; bağımsız evidence
  toplamaya devam et.
...
ettiği kanıt zaten elde edilmiş olmalı).
...
Controlled transformation (deterministic, öngörülebilir bir dönüşüm var mı?)
   ↓
Independent confirmation (farklı bir evidence kategorisiyle doğrulandı mı?)
   ↓
...
"beklenen sonuç göründü" SSTI'yi kanıtlamaz.
...
- **Medium indicator** — birden fazla engine'de görülebilir ama belirli
  ...
  çalışması → Java-family olasılığı yüksek, ama kesin değil).
...
{{6666*6666}} → 44435556 mı?
 ...
 │    weak indicator kombinasyonu) → §12.1/§12.3
...
- Baseline'ı N kez, aday payload'ı N kez ölç (aynı N).
- Sonucu median + IQR/MAD gibi bir istatistikle değerlendir.
...
- **Path Traversal / RCE (prototype pollution zinciri üzerinden,
  ...
  side_effect_level: state-changing — §0.2 etik sınırına tabidir.)*
...
- **RCE / confirmation:**
  ...
  *(Metadata — §14.2: payload_stage=P6, prerequisites:
  ...
  `sandbox=absent` (default); side_effect_level=read-only — yine de
...
- **Önemli negative indicator:** Go template dili doğrudan `+`/`*`
  ...
  ```
- **`html`/`js` fonksiyonları — text/template vs html/template ayrımı
  için kullanılabilir bir sinyal (düzeltilmiş açıklama):** `html` ve
  `js`, **yalnızca `text/template`'in** built-in fonksiyon listesinde
  bulunur (HTML/JS-escape eden yardımcı fonksiyonlardır). Resmi Go
  dokümantasyonu bu iki fonksiyonun **`html/template`'te kullanılamaz**
  olduğunu açıkça belirtir. Bu, doğrudan bir fingerprint sinyaline
  dönüştürülebilir:
  ```
  {{html "ssti"}}
  ```
  ...
  - **html/template'te:** `function "html" not defined` benzeri bir
    **hata** döner — bu hata mesajının kendisi, hedefin `html/template`
    kullandığına dair bir **Medium/Strong indicator**'dır.
...
- **text/template vs html/template ayrımı:** `html/template`
  ...
  `text/template` ise hiçbir escape yapmaz. Yukarıdaki `html`/`js`
